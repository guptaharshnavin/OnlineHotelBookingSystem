...
<html>
	...
	<body>
		...
		<center>
			...
			<br>
			<div>
				<center><h3>Reset Hotel Account Password</h3>
				<table>
					<tr>
						<th>Username : </th>
			...
			<br>
			<div>
				<center><h3>Hotel Account Log</h3>
				<table>
					<tr>
						<th>Hotel Name</th>
						<th>Email ID</th>
						...
						<?php
							$sqlone = "select * from hotel_log";
							$resultone = $conn->query($sqlone);

							if ($resultone->num_rows > 0) {
			    				while($row = $resultone->fetch_assoc()) {
			    					echo "<tr>";
									echo "<td>" .$row["HotelName"] ."</td><td>" .$row["EmailID"] ."</td><td>" .$row["Insert_Op"] ."</td><td>" .$row["Date_Of_Op"] ."</td>";
									echo "</tr>";
								}
			    			}
						?>
	...
	<script>
		function changeStatus(username,choice) {
        	...
        	xmlhttp.open("GET", "/OnlineHotelBookingSystem/Hotel_Manager_Pages/hotel_act.php?user=" + username + "&act=" + choice, true);
        	xmlhttp.send();
    	}
    	function resetpass(username) {
        	...
        	xmlhttp.open("GET", "/OnlineHotelBookingSystem/Hotel_Manager_Pages/hotel_reset.php?user=" + username, true);
        	xmlhttp.send();
    	}
	</script>
</html>
